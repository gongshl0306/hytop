"""dcutop command line interface.

    dcutop                    live curses TUI
    dcutop --once             one snapshot as plain text
    dcutop --json             one snapshot as JSON
    dcutop --frames N         headless TUI: render N frames, print the last
    dcutop -d 0,1             restrict to devices 0 and 1
    dcutop --backend mock     deterministic simulation without hardware
...
from dcutop.ffi.errors import DriverNotFoundError, DcutopError
from dcutop.collector import _restrict_processes as restrict_processes
...
from dcutop.report import snapshot_to_dict
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--version", action="version", version=f"dcutop {dcutop.__version__}")
    return parser
...
def render_once(snapshot: SystemSnapshot) -> str:
    ...
    lines.append(
        f"dcutop {dcutop.__version__}  host: {hostname}  "
...
def main(argv=None, backend_factory=make_backend, stdout=None, stderr=None) -> int:
    ...
    if args.interval <= 0:
        print("dcutop: --interval must be positive", file=stderr)
        return EXIT_USAGE
    ...
    try:
        ...
        print(f"dcutop: {err}", file=stderr)
        return EXIT_USAGE
    except DcutopError as err:
        print(f"dcutop: {err}", file=stderr)
        return EXIT_DRIVER
