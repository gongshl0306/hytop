...
from dcutop.tui.panels import TuiState, handle_key, render_frame, text_of
from dcutop.tui.theme import STYLE_NAMES
...
def _headless(collector: Collector, state: TuiState, frames: int, stdout,
    ...
    while collector.snapshot() is None:
        if time.monotonic() > deadline:
            print("dcutop: collector produced no snapshot in time", file=sys.stderr)
            return
        time.sleep(0.02)
