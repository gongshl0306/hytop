"""Keep the version consistent across pyproject.toml and hytop.__version__."""
...
class TestVersionConsistency(unittest.TestCase):
    def test_pyproject_matches_package_version(self):
        ...
        self.assertEqual(match.group(1), hytop.__version__)

    def test_console_script_entrypoint_exists(self):
        pyproject = (
            pathlib.Path(__file__).resolve().parents[1] / "pyproject.toml"
        ).read_text()
        self.assertIn("hytop = \"hytop.cli:main\"", pyproject)
